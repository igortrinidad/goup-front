<template lang="html">
    <div>
        <div class="container m-t-20">
            <div class="row">
                ...
                <div class="col-sm-12 text-center m-t-20">
                    <p class="f-300"  v-if="!comments.length">Esta receita ainda não possui comentários</p>
                    ...
                    <p v-if="!isLogged">Faça o login para comentar na receita</p>
                </div>
            </div>
        </div>
        ...
        <div class="container m-t-30" v-if="comments.length">
                    ...
                    <div class="card">
                        <div class="card-body card-padding text-center">
                            <div class="" v-for="(comment, commentIndex) in comments">
                                ...
                                <hr v-if="commentIndex + 1 < comments.length">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row" v-show="comments.length">
                <div class="col-sm-12">
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.newComment.meal_recipe_id = this.recipe.id
            this.getComments()
        },
        methods: {
            ...
            getComments: function(){
                ...
                this.$http.get(`meal/recipe/comment/list/${that.recipe.id}`).then(response => {
                    that.comments = response.data.comments;
                    that.pagination = response.data.pagination;
                    that.navigate_url = `meal/recipe/comment/list/${that.recipe.id}`;
            ...
            saveComment: function(){
                ...
                that.$http.post(`meal_recipe/comment/store`, that.newComment).then(response => {
            ...
            removeComment: function(comment, index){
                ...
                this.$http.get(`meal_recipe/comment/destroy/${ comment.id }`,).then(response => {
