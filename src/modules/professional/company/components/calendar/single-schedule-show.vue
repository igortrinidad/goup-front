                    ...
                    <div class="card-body card-padding p-b-5 text-center">
                        ...
                        <div class="row">
                            <button type="button" class="btn btn-xs p-5 p-l-10 p-r-10 m-r-5 m-b-5 btn-success"
                                    @click.prevent="confirmSingleSchedule">
                                Confirmar
                            </button>
            ...
            <div class="container">
                ...
                <div class="card" v-if="!placeholder.client">
                    <div class="card-header ch-alt text-center">
                        <router-link tag="div" class="cursor-pointer"  :to="{name: 'admin.companies.client.show', params: {company_id: $route.params.company_id, client_id: single_schedule.client.id}}">
                            <div class="picture-circle picture-circle-xs" :style="`background-image:url('${ single_schedule.client.avatar }')`"></div>
                            <h4 class="f-300 m-b-10">{{ single_schedule.client.full_name }}</h4>
                            <span class="f-300">{{ single_schedule.client.email }}</span>
                            <br>
                            <span class="label label-info f-16 p-5 f-400 m-t-5">Agendamento avulso</span>
                        </router-link>
                    </div>
                    <div class="card-body card-padding text-center">
                        ...
                        <div class="row">
                            <div class="col-md-12 col-xs-12">
                                <div class="border-inside-card">
                                    <span class="f-300 f-12">Observações</span>
                                    <p>{{single_schedule.observation}}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Helper Messagem To Alert Confirmed -->
                        <div class="row m-t-20" v-if="single_schedule.is_canceled">
                            <span class="f-300 m-t-30 f-danger">
                                Este agendamento foi cancelado por <strong>{{ single_schedule.canceled_by }}</strong>
                                em: <strong>{{ single_schedule.canceled_at | moment('DD/MM/YYYY') }}</strong>
                                às: <strong>{{ single_schedule.canceled_at | moment('HH:mm:ss') }}</strong>
                            </span>
                        </div>
                        <!-- / Helper Messagem To Alert Confirmed -->
    ...
    export default {
        ...
        methods: {
            ...
            getSingleSchedule() {
                ...
                that.$http.get(`professional/company/single_schedule/show/${that.$route.params.single_schedule_id}`)
            ...
            saveReschedule(){
                ...
                that.$http.post('professional/company/single_schedule/reschedule', that.reschedule_data)
                    .then(function (response) {
                        $('#modal-reschedule').modal('hide')
            ...
            cancelSchedule(schedule){
                ...
                that.$swal({
                    ...
                    that.$http.post('professional/company/single_schedule/cancel', cancel_data)
                        .then(function (response) {
