<template>
    <div class="m-t-30">
        <div class="first-container">
            ...
            <!-- Actions -->
            <div class="container" v-if="!placeholder.client && !placeholder.professional && !placeholder.plan">
                ...
                <div class="card">
                    <div class="card-body card-padding p-b-5 text-center">
                        ...
                        <div class="row">
                            <button type="button" class="btn btn-xs p-5 p-l-10 p-r-10 m-r-5 m-b-5 btn-success"
                        ...
                        <span class="row m-t-20">
                            <p>O horário indicado para remarcação deste agendamento é até <b>{{maxRescheduleTime().format('DD/MM/YYYY HH:mm:ss')}}</b>.</p>
                            <p>O horário indicado para cancelamento deste agendamento é até <b>{{maxCancelTime().format('DD/MM/YYYY HH:mm:ss')}}</b>.</p>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <!-- Client Infos -->
            <div class="container">
                <h3 class="f-300 text-center m-t-0 m-b-30">Cliente</h3>
                ...
                <div class="card" v-if="!placeholder.client">
                    <div class="card-header ch-alt text-center">
                        <div class="picture-circle picture-circle-xs" :style="`background-image:url('${ schedule.subscription.client.avatar }')`"></div>
                        <h4 class="f-300 m-b-10">{{ schedule.subscription.client.full_name }}</h4>
                        <span class="f-300">{{ schedule.subscription.client.email }}</span>
                    </div>
                    <div class="card-body card-padding text-center">
                        ...
                        <div class="row row-client-content">
                            ...
                            <div class="col-md-6 col-xs-6">
                                <div class="border-inside-card">
                                    <span class="f-300">Remarcado</span>
                        ...
                        <div class="row row-client-content">
                            <div class="col-md-6 col-xs-6">
                                <div class="border-inside-card">
                                    <span class="f-300">Data</span>
                                    <h4 class="m-t-5 m-b-0">{{schedule.date}}</h4>
                                </div>
                            </div>
                            <div class="col-md-6 col-xs-6">
                                <div class="border-inside-card">
                                    <span class="f-300">Horário</span>
            ...
            <div class="container">
                ...
                <card-placeholder v-if="placeholder.company" :circleImage="true" :onlyHeader="true">
                </card-placeholder>

                <div class="card" v-if="!placeholder.company">
                    <div class="card-header ch-alt text-center">
                        <div class="picture-circle picture-circle-xs" :style="`background-image:url('${ company.avatar }')`"></div>
                        <h4 class="f-300 m-b-0">{{ company.name }}</h4>
                    </div>
                </div>
            </div>
            <!-- / company Infos -->
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>
                            Selecione o novo dia para remarcar o agendamento do dia <strong>{{old_schedule_data.date}}</strong>
                        </p>
                    ...
                    <div class="modal-footer borderless">
                        <button class="btn btn-default" data-dismiss="modal" @click.prevent="cancelReschedule">Fechar
                        </button>
                    </div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-info" @click.prevent="saveReschedule" :disabled="!selectedSlot">
                            Remarcar
                        </button>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            ...mapActions(['addOnCompanyMonthSchedules', 'addOnCompanySelectedDaySchedules', 'removeFromCompanyMonthSchedules', 'removeFromCompanySelectedDaySchedules', 'setLoading']),
            ...
            getSchedule() {
                ...
                that.$http.get(`professional/company/schedule/show/${that.$route.params.schedule_id}`)
                .then(function (response) {
                    that.schedule = response.data.schedule
                    that.company = response.data.schedule.company;
                    that.category = response.data.schedule.category;
                    setTimeout(() => {
            ...
            getProfessionalCalendarSettings() {
                ...
                that.$http.post('professional/company/professional/calendar_settings/to_reschedule', {
            ...
            getCategoryCalendarSettings(){
                let that = this
                that.setLoading({is_loading: true, message: ''})
                that.$http.post('professional/company/category/calendar_settings/to_reschedule', {
                    company_id: that.reschedule_data.company_id,
                    category_id: that.reschedule_data.category_id,
                    date: that.selected_date
                })
            ...
            saveReschedule() {
                ...
                that.$http.post('professional/company/schedule/reschedule', that.reschedule_data)
                    .then(function (response) {
                        $('#modal-reschedule').modal('hide')
            ...
            handleReschedule(schedule) {
                ...
                if (old_schedule_date.isSame(schedule_date, 'day')) {
                    ...
                    that.addOnCompanySelectedDaySchedules(schedule)

                    if (schedule.company_id != that.old_schedule_data.company_id) {
                        that.removeFromCompanySelectedDaySchedules(that.old_schedule_data)
                    }
                } else {
                    ...
                    if (old_schedule_date.isSame(schedule_date, 'month')) {
                        ...
                        that.addOnCompanyMonthSchedules(schedule)
                        that.removeFromCompanySelectedDaySchedules(that.old_schedule_data)
                        that.getShedulesByCompany()
                    } else {

                        //if the new schedule month is different remove from current month
                        that.removeFromCompanyMonthSchedules(schedule)
                        that.removeFromCompanySelectedDaySchedules(that.old_schedule_data)
                    }
                }
            ...
            confirmSchedule(){
                ...
                that.old_schedule_data = _.cloneDeep(that.schedule)
                let confirm_data = _.cloneDeep(that.schedule)
                ...
                that.$swal({
                    ...
                    that.$http.post('professional/company/schedule/confirm', confirm_data)
                        .then(function (response) {
            ...
            cancelSchedule(schedule){
                ...
                that.$swal({
                    ...
                    that.$http.post('professional/company/schedule/cancel', cancel_data)
                        .then(function (response) {
