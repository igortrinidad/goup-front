<template>
    ...
    <div>
        ...
        <div class="container">
            ...
            <!-- Non clients -->
            <div class="row" v-if="non_clients.length">
                ...
                <div class="col-sm-4 col-xs-12" v-for="(client, $index) in non_clients">
                    <div class="card">
                        ...
                        <div class="card-body card-padding text-center">
                            ...
                            <!--Actions-->
                            <button class="btn btn-info btn-sm m-t-10" @click.prevent="clientSolicitation(client.id)">
                                Adicionar cliente
                            </button>
                            <!--Actions-->
                        </div>
            ...
            <div class="row">
                <h4 class="text-center">Clientes confirmados</h4>
                ...
                <div class="col-sm-4 col-xs-12" v-for="(client, $index) in clients_confirmed">
                    <div class="card">
                        ...
                        <div class="card-body card-padding text-center">
                            ...
                            <!--Confirmed client -->
                            <span class="d-block f-300 f-14 m-t-10" v-if="client.professionals[0].pivot.is_confirmed">
                            ...
                            <router-link
                                ...
                                v-if="client.professionals[0].pivot.is_confirmed"
                            >Ir para o dashboard do cliente
                            </router-link>

                            <hr>

                            <button class="btn btn-danger btn-block btn-sm m-t-10" @click.prevent="removeClient(client.id)">
                                Remover cliente
                            </button>
            ...
            <div class="row"  v-show="interactions.showUnconfirmedClients">
                <h4 class="text-center">Clientes não confirmados</h4>
                <p class="text-center" v-if="!clients_unconfirmed.length">Nenhum cliente não confirmado</p>
                <!-- v-for na .list-item -->
                <div class="col-sm-4 col-xs-12" v-for="(client, $index) in clients_unconfirmed">
                    <div class="card">
                        <div class="card-header ch-alt text-center">
                            <div class="picture-circle" :style="`background-image:url('${ client.avatar }')`"></div>
                            <h3 class="f-300 m-b-0">{{client.full_name}}</h3>
                        ...
                        <div class="card-body card-padding text-center">
                            ...
                            <button class="btn btn-info btn-sm disabled m-t-10"
                                    v-if="!client.professionals[0].pivot.is_confirmed && !client.professionals[0].pivot.requested_by_client">
                                Solicitação enviada
                            </button>
                            <button class="btn btn-info btn-sm m-t-10"
                                    v-if="!client.professionals[0].pivot.is_confirmed && client.professionals[0].pivot.requested_by_client"
                                    @click.prevent="acceptClientSolicitation(client.id)">
                                Aceitar solicitação
                            </button>
                            <button class="btn btn-danger btn-sm m-t-10"
                                    v-if="!client.professionals[0].pivot.is_confirmed"
                                    @click.prevent="removeSolicitation(client.id)">Excluir solicitação
                            </button>
                            <!--Unconfirmed client-->
                        </div>
                    </div>
            ...
            <div class="text-center m-t-20 m-b-20">
                <button class="btn btn-info" v-if="!interactions.showClientsDeleted" @click.prevent="interactions.showClientsDeleted = true">Exibir clientes removidos</button>
                <button class="btn btn-primary" v-if="interactions.showClientsDeleted" @click.prevent="interactions.showClientsDeleted = false">Esconder clientes removidos</button>
            </div>

            <div class="row" v-show="interactions.showClientsDeleted">
                ...
                <div class="col-sm-4 col-xs-12" v-for="(client, $index) in clients_deleted">
                    <div class="card">
                        ...
                        <div class="card-body card-padding text-center">
                            ...
                            <!--Unconfirmed client-->
                            <button class="btn btn-info btn-sm m-t-10"
                                    v-if="client.professionals[0].pivot.is_deleted && client.professionals[0].pivot.deleted_by_id != client.id"
...
<script>
    ...
    export default {
        name: 'admin-professional-clients',
        components: {
        ...
        methods: {
            ...
            getClients: function () {
                ...
                that.$http.post('professional/client/list', {professional_id: that.currentUser.id})
                    .then(function (response) {

                        that.total_clients = response.data.total_clients
                        ...
                        that.navigate_url = 'professional/client/list'
                    })
                    .catch(function (error) {
                        console.log(error)
                    });
            ...
            navigate(page) {
                ...
                that.$http.post(`${that.navigate_url}`, {
            ...
            search() {
                let that = this
                that.$http.post('professional/client/search', {
                    search: that.client_search,
                    professional_id: that.currentUser.id
                })
                    .then(function (response) {
            ...
            clientSolicitation(client_id) {
                ...
                if(!that.handleSubscriptionLimits()){
                    return false
                }

                that.$http.post('professional/client/solicitation', {
                    professional_id: that.currentUser.id,
                    client_id: client_id,
            ...
            saveClient() {
                ...
                that.client.professional_id = that.currentUser.id,
            ...
            removeClient(client_id){
                ...
                that.$swal({
                    title: 'Atenção',
                    text: 'Você tem certeza que deseja remover este cliente?',
                    type: 'warning',
                    ...
                    that.$http.post('professional/client/remove_solicitation', {
                        professional_id: that.currentUser.id,
                        client_id: client_id
                    })
                        .then(function (response) {
            ...
            reactivateSolicitation(client_id) {
                ...
                that.$http.post('professional/client/reactivate_solicitation', {
                    professional_id: that.currentUser.id,
                    client_id: client_id
                })
                    .then(function (response) {
            ...
            handleSubscriptionLimits(){
                let active_subscription = this.currentUser.subscription && this.currentUser.subscription.is_active
                let max_clients = this.currentUser.subscription.clients

                if(!active_subscription){
                    this.$swal('Assinatura expirada', 'Não é possível adicionar clientes sem uma assinatura válida.', 'error')
                    return false
                }


                if(active_subscription && (this.total_clients + 1) <= max_clients){
                    return true
                }

                this.$swal('Ops!', 'Você atingiu o limite máximo de clientes para a sua assinatura. Para adicionar mais clientes é necessário fazer o upgrade da assinatura atual.', 'info')
