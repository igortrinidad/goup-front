...
export default function(){
    ...
    // Retorna uma string com a primeira letra Uppercase e o resto Lowercase
    String.prototype.toTitleCase = function toTitleCase() {
        return this.replace(/\w\S*/g, function(txt){return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();});
    }

    // Retorna a cor equivalente a uma categoria (tem que ser o slug da categoria)
    String.prototype.colorByCategory = function () {
        if (this === 'pilates') {
            return { rgba: 'rgba(97, 93, 226, .7)', hex: '#615de2' }
        }
        if (this === 'personal') {
            return { rgba: 'rgba(0, 203, 178, .7)', hex: '#00cbb2' }
        }
        if (this === 'fisioterapia') {
            return { rgba: 'rgba(84, 212, 222, .7)', hex: '#54d4de' }
        }
        if (this === 'nutricao') {
            return { rgba: 'rgba(156, 64, 169, .7)', hex: '#9c40a9' }
        }
        if (this === 'crossfit') {
            return { rgba: 'rgba(108, 207, 95, .7)', hex: '#6ccf5f' }
        }
        if (this === 'massagem-estetica') {
            return { rgba: 'rgba(214, 94, 0, .7)', hex: '#d65e00' }
        }
        if (this === 'acupuntura') {
            return { rgba: 'rgba(248, 209, 99, .7)', hex: '#f8d163' }
        }
        if (this === 'academia') {
            return { rgba: 'rgba(204, 66, 127, .7)', hex: '#cc427f' }
        }
        if (this === 'demartologia') {
            return { rgba: 'rgba(193, 146, 226, .7)', hex: '#c192e2' }
        }
        if (this === 'cardiologia') {
            return { rgba: 'rgba(214, 70, 70, .7)', hex: '#d64646' }
        }
        if (this === 'ortopedia') {
            return { rgba: 'rgba(99, 176, 206, .7)', hex: '#63b0ce' }
        }

        return { rgba: 'rgba(136, 198, 87, .7)', hex: '#88C657' }
    }

    // Retorna a quantidade que cada valor de um array repete
    Array.prototype.countDuplicatedValues = function() {
        ...
        for (let i = 0; i < this.length; i++) {
            if (this[i] != current) {
                if (cnt > 0) {
                    count.push({ value: current, count: cnt })
                }
                current = this[i];
                cnt = 1;
            } else {
